...
func TestDocsMeta(t *testing.T) {
	...
	var resp []byte
	test.MockRouter.HandleFunc(apiDocsMeta, func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte(resp))
	}).Methods("POST")
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gotResp, err := DocsMeta(tt.args.ctx, tt.args.payload, tt.args.accessToken)
...
func TestSearchObject(t *testing.T) {
	...
	var resp []byte
	test.MockRouter.HandleFunc(apiSearchObject, func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte(resp))
	}).Methods("POST")
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resp = mockResp[tt.name]
			gotResp, err := SearchObject(tt.args.ctx, tt.args.payload, tt.args.accessToken)
			//fmt.Println(string(gotResp), err)
			if (err != nil) != tt.wantErr {
